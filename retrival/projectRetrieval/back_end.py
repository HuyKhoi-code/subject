...
def search_engine(new_df,k, last_pos) :  
  ...
  (distance, found_index) = neigh.kneighbors([tfidf_matrix[last_pos]])
  result = new_df.iloc[found_index.tolist()[0]]
  return result

########################################## begin - part 2 #####################################
def append_new_query_to_tail_df2(in_put):
  global main_df2
  new_df = main_df2.append({'Content': input_query(in_put)}, ignore_index= True)
  return new_df
...
def calc_tfidf(new_df) :
  count_vect = CountVectorizer(max_features = 10000)
  #X_train_counts = count_vect.fit_transform(df['Content'],y=None)
  X_train_counts = count_vect.fit_transform(new_df['Content'].apply(lambda X_train_counts: np.str_(X_train_counts)))
  ...
  return X_train_tfidf , tfidf_matrix

def result(X_train_tfidf,tfidf_matrix,new_df) :  
...
def heloMother(new_df):
  X_train_tfidf , tfidf_matrix = calc_tfidf(new_df)
  r = result(X_train_tfidf , tfidf_matrix, new_df)
  return r
