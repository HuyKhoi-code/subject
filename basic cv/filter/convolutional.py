...
for i in range (img_height - (ker_height -3)):
    for j in range (img_width - (ker_height -3)):
        # matrix la ma tran con trong img de nhan tich chap 
        matrix = np.zeros_like(kernel)
        matrix[:,:] = result[i: i + ker_height, j: j+ ker_width]
        sum = 0
        for x in range (ker_height):
            for y in range (ker_width):
                sum += matrix[x][y] * kernel[2-x][2-y]
        correlation[i][j] = sum 
print (correlation)
